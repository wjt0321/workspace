...
> [!warning]- 点击展开/收起 安全机制

安全是MoltBot设计的首要考量。项目实现了多层次的安全机制来保护用户数据和系统稳定性。

**默认安全策略**下，主会话中的工具直接在主机上执行，代理拥有完全的文件系统和进程访问权限。这一设计假设主会话由单一受信任用户使用，提供了最大的灵活性。

**群组会话沙箱**通过设置`agents.defaults.sandbox.mode: "non-main"`启用。在此模式下，非主会话（包括群组和频道中的会话）会在独立的Docker容器中执行。bash和进程工具也会在Docker环境中运行，有效隔离了潜在的恶意操作。

**工具白名单/黑名单**提供了细粒度的工具访问控制。默认情况下，沙箱模式允许以下工具：`bash`、`process`、`read`、`write`、`edit`、`sessions_list`、`sessions_history`、`sessions_send`、`sessions_spawn`；同时禁止以下工具：`browser`、`canva`等可能涉及敏感操作的工具。

**执行审批机制**允许用户审批或拒绝代理的特定操作请求。这一功能对于需要人工监督的关键操作（如系统命令执行）特别有用。
## 四、本地部署方案
