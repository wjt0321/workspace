...
1. **禁止自动 Push**: 除非收到明确的"上传"、"推送"、"push"或包含"远程仓库"字样的指令，否则严禁执行任何会将本地代码变更同步到远程仓库的 Git 命令。
2. **本地操作豁免**: 允许执行所有纯本地 Git 操作（add, commit, branch, merge, stash, log, diff, status）。
3. **明确上传触发**: 仅当指令包含 "上传到远程仓库" / "推送到远程" / "执行push" / "同步到远程" 时执行上传。
4. **安全确认**: 执行远程上传操作前，必须回复 "确认要上传到远程仓库吗？" 并等待确认。
5. **歧义处理**: 指令存在歧义时（如 "提交代码"），默认按本地操作处理。
