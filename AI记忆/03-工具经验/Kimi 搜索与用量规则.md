...
## 2026-06-07 补充：原样输出硬约束
- 当用户要求脚本输出“原样发送”时，不能只保留文字内容，还必须保留换行、缩进、进度条、分段布局与文案。
- 最终回复应使用能保留空白与换行的文本块；把输出压成一段也属于破坏布局。
- 即使脚本输出可被总结，也不得总结、改写、重排、表格化或提炼；失败时同样原样报告最终报错或失败原因。